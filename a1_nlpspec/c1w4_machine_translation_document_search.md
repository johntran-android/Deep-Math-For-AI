...
🔗 **Related:** [# \\*Side of the plane function\\*. The\\* result is a matrix\\* def \\*side_of_plane_matrix\\*(P, v):     dotproduct = \\*np.dot(P, v.T)\\*     # Get a boolean value telling if the value in the cell is positive or negative     sign_of_dot_product = np.\\*sign\\*(\\*dotproduct\\*)      return \\*sign_of_dot_product\\*](hash_tables.md#node-660)
